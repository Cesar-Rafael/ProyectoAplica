...
/* completar las instrucciones de su lenguaje */
%%
...
int yylex(void) {
	...
	while (!feof(arch)) {
		...
		if (isalpha(c)){
			...
			if (strcmp(lexema, "%%")) return INICIO_FIN_COMENTARIO_EN_BLOQUE;
		...
		// TODO (Manuel, Rafael): Check if it is a single character or a more complex string.
		// (Rafael, Manuel): á
